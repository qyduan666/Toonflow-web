    ...
    <div class="toolbar">
      <div class="leftTools">
        ...
        <t-upload
          ...
          style="display: none" />
        <t-button theme="primary" :disabled="selectedRowKeys.length === 0" @click="handleBatchUpdateDesc">
          批量更新描述{{ selectedRowKeys.length > 0 ? `(${selectedRowKeys.length})` : "" }}
        </t-button>
      </div>
      <div class="rightTools">
        ...
        <t-button theme="default" variant="outline" :disabled="tableData.length === 0" @click="toggleSelectAll(!isAllSelected)">
    ...
    <div class="listPanel">
      <div class="listHeader">
        <div class="headerInfo">
          ...
          <span v-if="selectedRowKeys.length > 0">已选择 {{ selectedRowKeys.length }} 个</span>
        </div>
      </div>
      ...
      <t-loading :loading="loading" text="加载 Skill 列表中...">
        ...
        <div v-else class="skillRows">
          <div v-for="row in tableData" :key="row.id" class="skillRow" :class="{ selected: selectedRowKeys.includes(row.id) }">
            <div class="rowSelect">
              <t-checkbox :checked="selectedRowKeys.includes(row.id)" @change="toggleSelect(row.id)" />
            </div>
            ...
            <div class="rowMain">
              <div class="rowTop">
                ...
                <div class="metaTags">
                  <t-tag theme="default" variant="light-outline">{{ row.type || "-" }}</t-tag>
                  <t-loading v-if="row.state === 0" size="small" text="生成中" />
                  <t-tag v-else :theme="getStateTheme(row.state)" variant="light-outline">
              ...
              <div class="rowBottom">
                <div class="bottomMeta">
                  <span class="monoText">MD5: {{ row.md5 ?? "-" }}</span>
...
function getStateLabel(state?: number) {
  ...
  if (state === 0) return "正在生成描述";
  if (state === -1) return "MD5 不匹配";
  if (state === -2) return "文件缺失";
  return "未知状态";
}
...
function getStateTheme(state?: number): "success" | "warning" | "danger" | "default" {
  ...
  if (state === -1 || state === -2) return "danger";
  return "default";
}
...
const isAllSelected = computed(() => tableData.value.length > 0 && tableData.value.every((item) => selectedRowKeys.value.includes(item.id)));
...
async function loadSkills() {
  ...
  try {
    ...
    tableData.value = data.list.map((item, index) => ({
      ...
      updatedAt: formatTime(item.createTime),
    }));
...
function toggleSelectAll(checked: boolean) {
  if (checked) {
    ...
    const merged = new Set([...selectedRowKeys.value, ...currentPageIds]);
    selectedRowKeys.value = Array.from(merged);
    return;
  }
  const currentPageIds = new Set(tableData.value.map((item) => item.id));
  selectedRowKeys.value = selectedRowKeys.value.filter((id) => !currentPageIds.has(id));
}
...
async function openSkillEditor(row?: SkillRow) {
  if (row) {
    ...
    try {
      const detailResponse = await axios.post("/setting/skillManagement/getSkillDetail", {
        id: row.id,
        path: row.path,
        name: row.name,
      });
...
function handleDelete(row: SkillRow) {
  const confirmDialog = DialogPlugin.confirm({
    ...
    onConfirm: async () => {
      try {
        await axios.post("/setting/skillManagement/deleteSkill", { id: row.id, name: row.name });
...
async function handleBatchUpdateDesc() {
  if (selectedRowKeys.value.length === 0) {
    window.$message.warning("请先选择 skill");
    return;
  }
  saveLoading.value = true;
  try {
    await axios.post("/setting/skillManagement/batchUpdateDescription", {
      ids: selectedRowKeys.value,
    });
    window.$message.success("批量更新描述成功");
    loadSkills();
  } catch (error: any) {
    window.$message.error(error?.message || "批量更新描述失败");
  } finally {
    saveLoading.value = false;
  }
}
...
async function handleBeforeUpload(file: UploadFile) {
  ...
  try {
    ...
    await axios.post("/setting/skillManagement/importSkill", {
